...
export class FeedDatabase {
  ...
  markFeedScanSuccess(name: string, scannedAt: string, lastArticleAt: string | null): void {
    this.db
      .query(`
        UPDATE feeds
        SET last_scanned_at = ?,
  ...
  insertArticle(input: InsertArticleInput): { inserted: boolean; id: string | null } {
    const id = randomUUID();
  ...
  markArticleRead(id: string): boolean {
    return this.db.query("UPDATE articles SET read = 1 WHERE id = ?").run(id).changes > 0;
  }

  markAllRead(feedName?: string): number {
    if (feedName) {
      return this.db.query("UPDATE articles SET read = 1 WHERE feed_name = ?").run(feedName).changes;
    }
